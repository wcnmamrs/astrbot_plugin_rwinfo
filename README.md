...
- **重试名唯一性**：每条消息多个房号时，重试名用各自探针编号（`ABAB探针01_1`、`02_1`...）构造，避免串号
- **直连无端口默认**：`room_info.py 18.216.139.119` 直连未带端口时默认 5123，不报错
- **自动撤回房间解析信息**：查询成功回传的房间信息与重试进度在 `recall_delay` 秒后自动撤回（错误信息与指令回复不撤回）；默认关，可用 `/铁锈撤回` 开关/设延迟
...
> 除帮助外，其余指令默认**仅管理员可用**（AstrBot 的 `PermissionType.ADMIN`），防止群内普通成员滥用。

| 指令 | 权限 | 说明 |
|------|------|------|
| `/铁锈查房帮助` | 所有人 | 显示本插件所有指令及用法 |
| `/铁锈全局解析 开\|关` | 🔒 管理员 | 全局总开关（默认开）；无参数时显示当前状态 |
| `/铁锈模式 白\|黑` | 🔒 管理员 | 名单模式：白=仅名单内放行；黑=名单内禁用（默认黑）；无参数时显示当前模式 |
| `/铁锈玩家列表 开\|关` | 🔒 管理员 | 是否显示玩家列表（默认关）；无参数时显示当前状态 |
| `/铁锈白名 +@3245987504` | 🔒 管理员 | 白名单加用户(私聊) |
| `/铁锈白名 +#123456` | 🔒 管理员 | 白名单加群 |
| `/铁锈白名` | 🔒 管理员 | 打印白名单 |
| `/铁锈黑名 +@3245987504` | 🔒 管理员 | 黑名单加用户(私聊) |
| `/铁锈黑名 +#123456` | 🔒 管理员 | 黑名单加群 |
| `/铁锈黑名` | 🔒 管理员 | 打印黑名单 |
| `/铁锈重试 [0-10]` | 🔒 管理员 | 设置探针名被过滤时的自动重试次数；无参数显示当前配置 |
| `/铁锈撤回 开\|关\|秒数(0-300)` | 🔒 管理员 | 自动撤回开关/延迟秒数；无参数显示当前配置 |
...
- 同一房号 10 秒冷却，避免重复解析
- 私聊消息：若统一会话标识中含群号且通过名单校验，会按群处理；纯私聊无群标识则不处理

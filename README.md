...
- **权限控制**：全局开关 + 白名单/黑名单模式，支持按**用户**（`@用户ID`，私聊/群聊发送者）与**群**（`#群号`）分别控制；白名单模式多键判定（如群聊 + 用户）需**全部**在白名单才放行，黑名单模式任一命中即拦截
...
| `/铁锈模式 白\|黑\|混合` | 🔒 管理员 | 名单模式：白=仅名单内放行；黑=名单内禁用（默认黑）；混合=用户个例优先（`@` 优先于 `#`，同名互斥）；无参数时显示当前模式 |
...
| `/铁锈黑名` | 🔒 管理员 | 打印黑名单 |
| `/铁锈私聊策略 白\|黑` | 🔒 管理员 | 混合模式私聊默认策略：白=跟随白名单（不在白名单拦截）；黑=跟随黑名单（在黑名单拦截）；无参数显示当前策略 |
| `/铁锈私聊开关 开\|关` | 🔒 管理员 | 是否启用私聊查房处理（默认开）；无参数显示当前状态 |
...
| `mode` | 名单模式（white=白名单 / black=黑名单 / mixed=混合） |
| `white_list` / `black_list` | 白/黑名单（用户/群）列表，仅 `mixed` 模式外使用 |
| `mixed_white_list` / `mixed_black_list` | 混合模式独立白/黑名单，切换回白/黑模式时白/黑名单保持原状 |
| `mixed_private_policy` | 混合模式私聊默认策略：`white`=不在白名单拦截 / `black`=在黑名单拦截 |
| `private_enabled` | 是否启用私聊查房处理（默认开） |
| `debug` | 是否开启调试日志 |
...
- 同一房号 10 秒冷却，避免重复解析
- 私聊消息：按发送者用户ID（`@用户ID`）进名单校验，通过才查房；群聊同时校验 `#群号` 与发送者 `@用户ID`（用户黑名单在群聊里同样生效）
